...
type
  ...
  public
    { Public declarations}
    procedure GetAgendamentoAtivo (const AToken:String);//requisição de agendamento
    procedure GetHistorico (const AToken:String);//requisiçãode hitorico com todos agendamentos
...
//data_confirmação ao invés de dataConfirmacao
...
//até criarmos a aparte que identifica o usuario no APP
//UserID := 1;
TTask.Run(procedure
begin
  LoadCliente(Token);
...
try
  ...
  try
    ...
    ShowMessage('Falha na autenticação:'+Resposta.Content); //falha de id ou cpf invalido
    end;
    except
